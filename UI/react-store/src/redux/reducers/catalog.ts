...
const catalogReducer = createReducer(initStates.catalog, (builder) => {
  ...
  // builder.addCase(ACTIONS.PRODUCTS_BY_IMAGE_GET_REQUESTED, (state, action: AnyAction) => {
  //   const payload = action.payload;
  //   state.productsByImageSearch=payload;
  // });

  builder.addCase(ACTIONS.PRODUCTS_BY_IMAGE_SUCCESS, (state, action: AnyAction) => {
      const payload = action.payload;
      state.productsByImageSearch=payload;
  });
